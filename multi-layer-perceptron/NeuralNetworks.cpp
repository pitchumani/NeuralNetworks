...
int main() {
    ...
    std::cout << "== XOR Gate Example (using MultiLayerPerceptron) ==" << std::endl;
    // create a MultiLayerPerceptron for XOR gate
    
    MultiLayerPerceptron xor_gate({2, 2, 1}, 1.0, 0.5);
    // set weights for the XOR gate
    xor_gate.setWeights({
        {{-10, -10, 15}, {15, 15, -10}}, // Weights for the first hidden layer (distinct weights for each neuron)
        {{10, 10, -15}}                // Weights for the output layer: these values are chosen to combine the outputs of the hidden layer neurons to produce the XOR logic
    });
    // Print the hard-coded weights for the XOR gate
    xor_gate.printWeights();

    // Test the XOR gate with all combinations of inputs
    std::cout << "XOR(0, 0) = " << xor_gate.run({0, 0})[0] << std::endl;
    std::cout << "XOR(0, 1) = " << xor_gate.run({0, 1})[0] << std::endl;
    std::cout << "XOR(1, 0) = " << xor_gate.run({1, 0})[0] << std::endl;
    std::cout << "XOR(1, 1) = " << xor_gate.run({1, 1})[0] << std::endl;
